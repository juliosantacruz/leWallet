...
const expenseMonth = (element:string) => parseInt(element.split("-", 2)[1]) - 1;
...
export default function CardSummary( {dateCard}:any) {
  ...
  monthExpenses.forEach((expense)=> arrayExpensesAmount.push(Number(expense.amount)))
  const expensesAmount = arrayExpensesAmount.reduce((a,b)=> a+b,0)
  console.log(arrayExpensesAmount)
  ...
  monthIncomes.forEach((expense)=> arrayIncomesAmount.push(Number(expense.amount)))
  const incomeAmount = arrayIncomesAmount.reduce((a,b)=> a+b,0)
  console.log(arrayIncomesAmount)
  console.log('income', monthIncomes)
